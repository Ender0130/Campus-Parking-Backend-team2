...
# - Stores parking lot data (name, campus, spots, status, last update)
...
class ParkingLot:
    ...
    def __init__(
        ...
        if current_status not in self.POSSIBLE_STATUSES:
            raise ValueError(
                f"Invalid status: {self.current_status}. Must be one of {self.POSSIBLE_STATUSES}."
            )
    ...
    def can_user_report_again(self, user_id, cooldown_minutes=10):
        ...
        Returns True if no report from this user in the last cooldown_minutes.
        """
        cutoff_time = datetime.now() - timedelta(minutes=cooldown_minutes)

        for report in self.reports:
            if report.reporter == user_id and report.timestamp >= cutoff_time:
                return False
    ...
    def update_status_from_recent_reports(self, window_minutes=15):
        ...
        print(f"Status counts for {self.campus} - {self.name}: {status_counts}")

        if status_counts:
            most_common_status, count = status_counts.most_common(1)[0]
            if count >= 3 and most_common_status != self.current_status:
                print(
                    f"Updating {self.campus} - {self.name} to {most_common_status} "
                    f"(majority {count} votes)"
                )
    ...
    def to_dict(self):
        return {
            ...
            "total_spots": self.total_spots or 0
        }

    def __str__(self):
        time_str = self.last_updated.strftime("%I:%M %p")
        return (
            f"{self.campus} - {self.name}: {self.current_status} "
